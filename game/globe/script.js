...
(function() {
    ...
    function createGlobeWithTexture(colorTexture, bumpTexture) {
        ...
        // Initialize grid after Earth is created
        if (!gridInitialized && typeof initHolographicGrid === 'function') {
            try {
                initHolographicGrid();
                gridInitialized = true;
    ...
    function initHolographicGrid() {
        // This function is now fully implemented in holographic-grid.js
    }
            ...
            setTimeout(() => {
                ...
                if (intersects.length > 0) {
                    ...
                    if (gridOverlay && gridOverlay.material.uniforms) {
                        gridOverlay.material.uniforms.selectedCell.value.x = -1;
                        gridOverlay.material.uniforms.selectedCell.value.y = -1;
                    }
                }
    ...
    function animate() {
        ...
        try {
            ...
            try {
                ...
                if (typeof updateHolographicGrid === 'function' && gridInitialized) {
        ...
        if (window.navigator && window.navigator.serviceWorker) {
            try {
                ...
                if (!isRestrictedEnvironment) {
                    ...
                    // Use a normal script instead of blob URL which causes errors in some environments
                    const swCode = `
                    ...
                    // Create a safe minimal service worker that won't try to use blob URLs
                    try {
    ...
    function updateSunPosition() {
        try {
            ...
            if (!window.directionalLight) {
                // Don't warn repeatedly - just check quietly
                return;
            }
            
            // existing code...
        } catch (e) {
            // Reduce error spam by not logging this repeatedly
