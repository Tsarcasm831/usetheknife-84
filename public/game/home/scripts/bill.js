...
/**
 * Loads all animation GLBs for Bill and attaches them as actions.
 * @param {THREE.Scene} scene
 * @param {Object} animPaths - Map of animation type to GLB path
 * @param {function(number):void} [onProgress]
 * @returns {Promise<THREE.Object3D>}
 */
export async function createBill(scene, animPaths, onProgress) {
    ...
    for (const [anim, path] of Object.entries(animPaths)) {
        try {
            ...
            console.warn(`Bill: Failed to load animation ${anim} from ${path}`);
        }
    }
    if (baseObj) {
...
export function updateBill(obj, deltaTime) {
