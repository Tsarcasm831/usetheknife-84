...
/**
 * Creates and places a GLB-based Agave Plant at a specified grid location.
 * @param {THREE.Scene} scene - The scene to add the agave to.
 * @param {number} [gridX=77] - Grid X coordinate.
 * @param {number} [gridZ=96] - Grid Z coordinate.
 * @returns {Promise<THREE.Group>} The created agave group.
 */
export function createAgavePlant(scene, gridX = 77, gridZ = 96) {
    ...
    return loadModel('/assets/static/flora/agave_plant_0502171941.glb')
        .then(gltf => {
            const plant = SkeletonUtils.clone(gltf.scene);
            plant.name = 'AgavePlant';
        ...
        .catch(err => console.error('Failed to load agave plant GLB:', err));
}
